...
const Header = (props) => {
  ...
  const { user } = UseUser(); //forogt invoking it
  const player = usePlayer();

  const handleLogout = async () => {
    const { error } = await supabaseClient.auth.signOut();
    //reset any playing songs, so that on logging out it stops
    player.reset();
  ...
  return (
    <div
      className={twMerge(
        //`h-fit bg-gradient-to-b from-cyan-700 p-6`,
        //from secondary
        `h-fit bg-gradient-to-b from-primary_bright p-6`,
            ...
            <>
              ...
              <div>
                <Button
                  ...
                  onClick={authModal.onOpen} //dn execute it ()
                >
                  Sign Up
                </Button>
              </div>
